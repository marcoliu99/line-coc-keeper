...
- **Keeper 目前忙碌**（一次 AI 呼叫進行中），玩家訊息進來——加入「下一批」的佇列，不再各自排隊
  等著跑單獨一次。目前這次 AI 呼叫結束後，若佇列非空，**幾乎立刻**把目前累積的整批送出——不是
  另外開一個多秒鐘的收單視窗，只給一個非常短的寬限期，接住「幾乎跟 flush 同一瞬間送到」的訊息
  （玩家正在打字，晚了零點幾秒送出），不讓它平白錯過這一批、自己又開新的一輪。
  - `MAX_BATCH_WAIT_SECONDS`（**預設 0.7 秒**，`app/config.py` 環境變數可調——2026-09-20
    review 修正：舊版預設 3 秒、講成「合理範圍 2–5 秒」，等於每次都可能真的乾等 2-5 秒才送出，
    跟「幾乎立刻送出」自相矛盾，也直接抵銷批次化想降低延遲的目的；已改成刻意很短的值，正常情況
    下根本不會真的等滿——多數時候是被下面的 `MAX_BATCH_SIZE` 或緊接著抵達的下一輪訊息提前打斷，
    這個數字只是「萬一什麼都沒有發生」時的保底上限，不是常態）：批次在準備要 flush 的那一刻，
    給一個最多 `MAX_BATCH_WAIT_SECONDS` 的寬限期，超過就照原佇列內容立即送出，不再延後。
...
- `MAX_BATCH_WAIT_SECONDS` 預設 **0.7 秒**（2026-09-20 review 修正，原本 3 秒／2–5 秒範圍會讓
  每輪批次幾乎都平白多等好幾秒，抵銷批次化降低延遲的目的——見「批次視窗」一節），`app/config.py`
  環境變數可調，是萬一沒有任何提前喚醒訊號時的保底上限，正常情況不會真的等滿。
