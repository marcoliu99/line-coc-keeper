...
### 52. `find_room_by_text` 模糊比對加剪枝，過濾掉大部分不可能命中的滑動視窗
...
- **這個專案現在怎麼做**：呼叫 `SequenceMatcher.ratio()` 之前，先檢查這個滑動視窗跟房間名稱**有沒有至少一個共同字元**（`set(name) & set(window)`）。這不是近似、不會改變任何結果——`SequenceMatcher.ratio()` 算的是最長共同子序列，如果兩個字串連一個字元都不重疊，比對結果保證是 0，本來就不可能通過門檻；用集合交集當前置篩選，只是提早跳過那些「算了也是白算」的視窗，不會漏掉任何原本會命中的案例。實務上，劇本文字（常常是英文原文或跟房間名稱完全無關的敘述）跟房間名稱重疊的機率很低，大部分視窗會直接被濾掉。
- **實測過**：
  - 正確性：拿同一組房間清單（10 個常見中文房間名稱）跑 4 種輸入文字（完全沒有中文字重疊的英文劇本片段、包含完整房間名稱的句子、需要模糊比對的近似房間名稱、系統自產的檢定結果敘述），修改前後回傳的房間（或沒有命中）完全一致。
