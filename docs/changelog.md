...
### 59. 角色卡與字典系統：檔案分流、自動補齊、跨語言比對、擇優融合（模組一～五）
...
### 60. 官方技能中文譯名改用專案擁有者提供的對照表，並補上 `extract_pregens` 的技能字典查表
...
### 61. 修正 pregens 角色池的兩個上傳順序 bug，PDF 上傳改成立即抽取內建角色卡
...
- **實測過**：
  ...
  - 正式環境資料庫全程沒有被動到。
### 63. 修正 LLM 抽取角色卡時兩個「安靜地用錯資料」的既有小缺口
...
### 64. 新增 `role_` 角色卡上傳範本，並用官方屬性表核對 `BASE_SKILLS`
...
### 65. 模組七：修正「整備階段防誤觸」真的沒做到的 bug，`/coc start` 新增完整度自動修復＋開場就緒名冊
...
### 66. 就緒名冊補上 HP/SAN/裝備，新增劇本開場白內建檢定的自動支援
