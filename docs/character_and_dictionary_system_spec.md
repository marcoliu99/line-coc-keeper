...
⚠️ 實作備註（2026-09-17 討論定案）：核對 `app/commands.py` 的 `/coc pc`／`/coc usepregen`，
確認 `state.pregens`（角色池）跟 `state.characters`（真正出戰的角色）現在是完全獨立的兩份
資料——`pregen_extractor.pregen_to_character` 在玩家認領當下把 pregen 資料**複製**成一個新的
`Character`，複製之後兩邊就沒有任何關聯，GM 之後重傳角色卡更新角色池，完全不會影響已認領、
正在玩的角色。也就是說，「已出戰動態進度保護」這條規則在現有架構下**不會被觸發**——要讓它
有意義，必須新增「重傳角色卡回頭更新已認領角色」這個現在不存在的行為。

討論後決定**縮小範圍**：擇優融合只發生在角色池裡**還沒被任何玩家認領**的 pregen 之間，已經
認領、正在玩的角色完全不受任何重傳影響（不新增反向同步機制）。「已出戰動態進度保護」這條
規則因此在這次改動裡**不適用**，不用實作。

**選 A 範圍下的具體設計**：
1. **來源標記**：`parse_role_sheet_text`（`role_` 上傳）跟 `extract_pregens`（劇本 PDF 抽取）
   各自在產生 pregen 時標記 `source: "manual"` / `source: "llm_extracted"`，供融合規則判斷哪筆
   是手傳卡、哪筆是 PDF 抽取版。
2. **去重比對**：現有「職業字串相同就覆蓋」換成模組三的身分比對（指紋／技能／姓名字典三道
   閘門），比對範圍是整個角色池，不再限定同職業才比對——這是修正「同職業覆蓋踩踏」bug 的
   真正做法（用身分判斷，不是用職業字串）。
3. **比對成功時的欄位融合**：屬性／衍生值以 `manual` 為主（`manual` 缺的欄位從
   `llm_extracted` 補）；技能取聯集，重複技能以 `manual` 數值為準；秘密動機從
   `llm_extracted` 繼承；背景／notes 以 `manual` 為主；玩家綁定留給模組六一起討論。
4. **來源相同的例外**：比對成功的兩筆若剛好來源相同（例如 GM 重傳了兩次手打角色卡），沒有
   「手傳/PDF」可以區分優先順序，直接用新的那筆整個覆蓋舊的（跟現有行為一致，只是去重依據
   從職業字串換成身分比對）。
