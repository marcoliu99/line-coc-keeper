...
5. **需要檢定時，預設由玩家觸發角色骰，不是 Keeper 代替玩家決定結果**：玩家描述要做什麼後，守密人會呼叫檢定工具建立待處理檢定；玩家按 Discord 按鈕或輸入 `/coc check`，程式才擲出 d100、判定成功等級並把結果交回守密人敘事。技能、攻擊、閃避、反擊、SAN 與重傷 CON 都遵守這個預設。
   ...
   - **擲骰結果很接近成功時，系統會主動問要不要花 Luck 買到更好的結果**（COC7e 的 Luck 花費機制）：玩家先完成角色骰，Discord 再附上 Luck 選擇按鈕；玩家也可以輸入 `/coc luck skip` 或 `/coc luck regular`／`hard`／`extreme`。理智檢定不適用這個機制，只適用一般技能檢定和戰鬥相關檢定（含閃避／反擊）。
