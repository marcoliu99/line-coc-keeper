...
5. 打起來的時候，守密人通常會自己判斷該開戰、依序處理每個人的回合，**嚴格按照 DEX 排出的先攻順位進行**——DEX 不同的人行動跟敘述都要照順序來，只有 DEX 剛好相同的人才會敘述成同時行動（細節見 [docs/keeper_skill.md](keeper_skill.md) 的「戰鬥規則」段落）；也可以自己手動控制：
