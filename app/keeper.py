...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "把戰鬥推進到下一位戰鬥員的回合（已倒下的會自動跳過）。每次處理完一位戰鬥員的"
            "行動後都必須呼叫這個工具，不可以自己心裡默默跳過。"
            "如果現在這位戰鬥員的行動需要先完成一個檢定或動作才算處理完（例如逃脫、撲救"
            "掩體、掙脫束縛等），要先呼叫對應的機制工具（例如 skill_check）取得結果，不要"
            "在還沒解決之前就呼叫這個工具跳過；也不要因為上一次呼叫沒有得到你想要的結果，"
            "就重複呼叫這個工具期待狀態自己改變——這個工具只會推進到下一位，不會幫你解決"
            "任何檢定。"
        ),
        "input_schema": {"type": "object", "properties": {}},
    },
