...
- **還是有的限制**：
  ...
  4. 索引依 `group_id` 快取在記憶體裡（跟著程式行程活，不寫到 `data/groups/*.json`），重啟 LINE/Discord 常駐行程後第一次查詢會重建一次索引（BM25 是純 CPU；有金鑰的話 embeddings 那段會重新打一次 API，不算貴，但確實是重啟後的額外成本）。
...
- **之後要擴充的話**：400 字的分塊目標是憑經驗抓的常數，還沒有針對真實劇本的分塊效果做過系統性調校；索引目前還是每次重啟都要重建，之後可以考慮把索引（含 embeddings）也持久化到磁碟，重啟後不用重算。
...
- ~~`MAX_LOG_TURNS`／`MAX_SCENARIO_CHARS` 分別限制了守密人能看到的對話紀錄長度和劇本長度，超過的部分會被截掉；長篇戰役玩到後期，比較早期的劇情細節守密人可能會忘記，很長的劇本 PDF 後段內容也可能讀不到~~ 部分緩解：
  ...
  - **滾動摘要本身有個隱憂，已經另外補了一層安全網**：「舊摘要 + 新對話 → 更新摘要」是不斷疊加壓縮，長期下來（很多次裁切之後）細節有可能被壓縮掉——這不是理論疑慮，是滾動摘要這個做法天生的限制。新增 `app/memory_rag.py`：每次觸發裁切、把舊對話摺進 `campaign_summary` 的**同一時間**，也把那段對話的**原始文字**（不是摘要）向量化存進一個獨立的持久化檔案（`data/groups/<id>_memory.json`，架構整個抄 `app/scenario_rag.py` 現有的 BM25 + OpenAI embeddings 混合檢索，但沒有共用程式碼——`scenario_rag.py` 是正式環境跑著的功能，不想為了共用一點程式碼去動它、冒風險），新增的 `search_memory` 工具讓 Keeper 在摘要和最近對話都查不到某個具體細節時，直接語意搜尋回那段原始對話。實測過一個刻意設計來驗證這個隱憂的案例：塞一個容易在壓縮裡流失的具體細節（懷錶刻的年份「1897」、指針停在「三點十七分」），連續觸發兩次裁切（兩輪滾動摘要疊加壓縮）之後，直接問 Keeper 這個細節——確認就算 `campaign_summary` 那個回合真的把細節壓掉了，`search_memory` 依然能從原始存檔裡正確撈回來，也真的讓 Keeper 在完整對話裡自己判斷該呼叫這個工具、答對了年份和時間。
    - **這層安全網還有的限制**：`data/groups/<id>_memory.json` 只會一直長大，沒有上限、沒有過期機制，非常長的戰役理論上會累積很多記憶片段（檢索速度目前還沒實測過真的很長之後會不會變慢）；每個片段是整段原始對話文字加一組 embedding 向量，比純文字摘要佔用的空間大得多；沒有 OPENAI_API_KEY 時這層會退化成純 BM25 關鍵字比對（跟 `scenario_rag.py` 同樣的降級邏輯），語意檢索的效果就沒有了，但功能還是能用。
