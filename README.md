...
5. 守密人主持風格上的幾個約定（都是 prompt 層面的行為，沒有對應指令）：
...
6. **地圖引擎（Map/Scene Engine）**：如果劇本裡有平面圖（例如「燈塔一樓」），上傳時會自動偵測並拆成結構化的房間圖。之後玩家講「我進入燈塔，開始檢查右手邊第一個房間」這種話，系統會在呼叫 LLM **之前**，先用房間圖＋目前朝向算出「右手邊第一個房間」實際是哪一間，把算出來的房間名稱與描述直接餵給守密人、並告訴它不准自己另外猜——不是讓 LLM 自己讀地圖描述文字去猜方位（這是之前地圖方向會搞錯的根源）。
