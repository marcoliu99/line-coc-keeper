...
2. 每位玩家建立角色，三選一：
   ...
   - **使用劇本內建的預製角色**（如果 PDF 有附）：
     ...
     - **除了讓 LLM 自動從 PDF 抽取，也可以自己手動上傳角色卡文字**，繞過 LLM 抽取可能出的錯：把角色卡整理成純文字檔（`.txt` 或 `.md`），依照【角色資料】／【屬性】／【技能】／【角色背景】…… 這種區塊格式撰寫（範例格式見 `app/pregen_extractor.py` 的 `parse_role_sheet_text` docstring），**檔名開頭加上 `role_`**（例如 `role_05_狠角色.txt`）直接傳到群組/頻道，就會解析成一位預製角色加進 `/coc pregens` 的清單；只有 9 項基礎屬性和技能會從文字裡解析，HP/MP/SAN/MOV/傷害加值/體格一律由公式從屬性推算（COC7e 官方角色卡本來就是這樣算出來的，不會因為分開算而兜不起來）；同職業重傳一次會直接覆蓋舊的，不會累積重複。
...
- **之後要擴充的話**：把 7 點門檻改成可設定值；之後如果要做「场景結束恢復 Luck」之類的規則，可以加一個對應工具讓守密人呼叫。
