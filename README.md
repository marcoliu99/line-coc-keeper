...
- `data/groups/*.json` 是整檔讀出、整檔覆寫，沒有加鎖。如果同一個群組有兩個人幾乎同時打字，兩個請求會各自讀到同一份舊狀態、各自處理完再各自存檔，後存檔的會整份覆蓋掉先存檔的——例如兩人幾乎同時觸發技能檢定，其中一次的 HP/SAN 變化可能就這樣不見。人多手快的團比較容易踩到；之後可以加一個 per-group 的鎖把同群組的訊息序列化處理來解決。
...
- `MAX_LOG_TURNS`／`MAX_SCENARIO_CHARS`（見 `app/config.py`）分別限制了守密人能看到的對話紀錄長度和劇本長度，超過的部分會被截掉；長篇戰役玩到後期，比較早期的劇情細節守密人可能會忘記，很長的劇本 PDF 後段內容也可能讀不到。
