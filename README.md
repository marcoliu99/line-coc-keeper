...
- **這個專案現在怎麼做**：新增 `app/scenario_index.py`，思路照抄 [coc-kp-host](https://github.com/SumanasJ/coc-kp-host) 的「模組框架.md」概念（見 `docs/references/prep_persistence.md`）：一次強制工具呼叫（跟 `pregen_extractor.py` 一樣走 `LLM_PROVIDER` 分派，不寫死供應商）從劇本文字抽出：
  ...
  - **地點索引**：名稱、別名、一兩句摘要、頁碼。
  - 新增指令 `/coc index`——跟 `/coc pregens` 同一種「手動觸發、抽取一次就快取」的模式，不是每次上傳 PDF 就自動跑一次（避免每個群組不管用不用得到都多付一次 LLM 呼叫的成本）。結果存進 `GroupState.scenario_npc_index`／`scenario_location_index`，重新上傳新劇本 PDF 會清空（跟 `pregens` 同一套清空邏輯，避免舊劇本的怪物數值污染新劇本）。
  - 抽出的索引被塞進 `app/keeper.py` `_build_static_prompt` 的（有 prompt caching 的）靜態區塊，明確要求「同一隻怪物/NPC 全場只能有一組數值；如果索引把同一種生物分成不同條目，要先確認清楚眼前是哪一條目，再照那個條目的數值呼叫 `add_npc_to_combat`，不要混用不同條目的數字，也不要自己另外編一個」。索引是空的（還沒跑過 `/coc index`）時這整段完全不出現，行為跟這個功能出現之前一模一樣。
...
- **還是有的限制**：跟 `pregens` 一樣完全靠系統提示詞的自律要求 Keeper 遵守索引裡的數值，沒有程式碼強制檢查 `add_npc_to_combat` 的參數是否真的跟索引一致；索引只在明確跑過 `/coc index` 之後才存在，劇本上傳後如果沒人記得跑這個指令，行為就跟這個功能不存在時一樣（不會主動提醒）。
- **之後要擴充的話**：`/coc pdf` 上傳完成的提示訊息可以順便建議「劇本裡如果有多種怪物、想避免數值前後不一致，可以跑一次 /coc index」；`add_npc_to_combat` 的工具本身也可以考慮做成先查索引、找不到才讓 Keeper 自己填數值的兩段式設計，而不是純粹靠提示詞要求。
