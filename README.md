...
- **這個專案現在怎麼做**：三種都做了，而且 `/coc pc` 快速生成現在會參考「這份劇本」的內建角色卡，不是完全套用固定表：
  ...
  - `/coc pregens` + `/coc usepregen`：`app/pregen_extractor.py` 會請 Claude 掃一次已載入的劇本文字，把裡面明確寫出的角色卡（屬性、技能、背景、秘密目標）抽成結構化資料供選用；規則是「只回報文本裡寫的數值，不自己編」，缺漏的欄位（例如劇本沒寫 MP）才會用公式補上。`/coc pregens` 的清單會標示哪些已經被選走（`/coc pc` 或 `/coc usepregen` 選中一次就記住是誰選的，`/coc usepregen` 也會擋下別人重選同一位）。抽取結果會快取在 `state.pregens`，**換一份新劇本上傳時會自動清空**，不會把舊劇本的角色卡誤用到新劇本上（這是實測時真的發生過的問題，已修正）。
...
- **還是有的限制**：
  ...
  3. 選走的角色目前沒有「放棄／讓出」的指令，選錯了只能請該玩家自己再用 `/coc pc` 或 `/coc usepregen` 選別的（原本選的那個仍會顯示已被他選走，卡在那邊），要真正釋放給別人選，目前得手動編輯存檔或跑 `/coc newgame` 重來。
