...
- **還是有的限制**：跟這份設計文件本來就承認的一樣，只做了「遊戲中途宣稱攜帶物品」的動態審查；「建角時的靜態審查」（`/coc pc`／`/coc create`／`/coc usepregen` 建角流程問隨身物品、審查後記進角色卡）完全沒做，建角時還是不會特別問或審查隨身物品。跟這個專案其他敘事層規則一樣，完全靠提示詞自律，沒有程式碼強制檢查 Keeper 是不是真的每次都四項都查過。
- **之後要擴充的話**：如果之後真的要做建角時的靜態審查，`docs/references/carry_audit.md`「建角時的靜態審查」一節已經寫好規格，可以直接照抄。
