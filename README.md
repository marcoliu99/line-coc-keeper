...
- **這個專案現在怎麼做**：`app/keeper.py` 的 `skill_check`／`sanity_check` 工具改成只「請求」一次檢定——記錄要檢定的技能、目標值、獎懲骰（或 SAN 損失公式），存進 `GroupState.pending_checks`，不會真的擲骰，也不准守密人自己編一個結果。守密人呼叫完工具後只能敘述「需要做這個檢定」的當下場景，停在那裡等玩家真的擲骰。玩家輸入 `/coc check`（不用加參數就會用守密人剛剛指定的技能/目標值）就會由程式碼（`app/dice.py`，跟原本一樣的判定邏輯）擲出真正的 1d100、算出成功等級，結果立刻公開顯示在群組裡；接著這個「已經確定的結果」會餵回守密人，請它根據這個既定事實繼續敘事，不能重新判定或改判。也可以不等守密人先開口，自己主動打 `/coc check 技能名 [獎勵骰數] [懲罰骰數]` 檢定。
  - **多選一的檢定**（例如近戰被攻擊時要選「閃避」還是「反擊」）：`offer_check_choice` 工具讓守密人一次給至少兩個選項，`/coc check <選項名稱>` 指定要選哪一個；選錯字或沒指定的話會列出可選項目、不會直接放棄整個待處理的檢定。這解決了 COC7e 規則裡「防守方要自己選擇閃避或反擊」這個典型情境——但**只到「讓玩家選要骰哪個技能」這一步，不是完整的對抗檢定**：兩邊實際比較成功等級、判定誰贏，還是要靠守密人敘事判斷，沒有自動比較雙方骰出結果的機制（細節見 `docs/references/rules_reference.md` 的「對抗檢定」落差說明）。標籤裡帶「反擊」字樣的選項，技能值會由程式碼自動減半（等同困難難度）——反擊同時要攻防兩用，房規判定比純閃避難一級，這是寫死在 `app/keeper.py` 裡的規則，不是每次都靠守密人自己拿捏。
...
### 13. 槍械彈藥追蹤（「目前遊戲狀態」裡會變動的數字之一）

- **這個專案現在怎麼做**：`Character.weapons` 是一個 `{槍名: {ammo, ammo_max}}` 的字典，只追蹤**已經確定持有、標了彈容量的槍械**目前剩幾發——不是完整的武器/裝備系統（那是 `docs/references/carry_audit.md` 的範圍，還沒做）。`role_` 開頭的角色卡上傳（見「玩法」段落跟上面第 11 項）會自動從【武器】區塊解析：每個武器子區塊只要有寫「彈容量：N」就登記成一把有彈藥追蹤的槍，初始彈數＝彈容量（滿彈開局）；沒寫彈容量的（近戰、投擲、徒手）不會出現在這個欄位，武器的完整敘述（傷害、射程、故障值……）還是照舊整段存進 `notes`，不會漏掉。`/coc sheet`／守密人每回合看到的角色卡現在會多一行「彈藥：.38 左輪手槍 3/6」這種格式；新增的 `adjust_ammo` 工具讓守密人在角色真的開槍時扣彈（`delta` 通常是 -1）、裝填/換彈匣時補滿（`reload_full: true`），系統提示詞也加了對應規則要求守密人確實呼叫。用真實角色卡（`06_巡警.txt` 的 `.38 左輪手槍`，彈容量 6）實測過完整流程：解析出彈容量、角色卡正確顯示 `6/6`、呼叫 `adjust_ammo` 開槍後正確扣到 `5/6`、打光後不會扣出負數、裝填後正確補滿；也真的讓 Keeper 在完整對話（不是直接呼叫工具，是真的餵一句「阿凱對歹徒開了一槍」）裡自己判斷該扣彈，成功正確呼叫。
- **還是有的限制**：
  1. 只有透過 `role_` 角色卡上傳才會自動解析出彈藥欄位；`/coc pc`／`/coc create`／劇本 PDF 自動抽取的預製角色目前都不會有這個欄位（沒有武器資料可以解析），要嘛手動之後再補一個工具/指令直接設定，要嘛就是完全沒有彈藥追蹤，一樣靠守密人敘事拿捏。
  2. 沒有做「彈匣沒打光就要手動退膛、只想補幾發不是整匣重裝」這種更細的規則，`reload_full` 只有「直接補滿」一種選項。
  3. 沒有故障（Malfunction）機制——彈藥登記時「故障值」欄位一樣被丟進 `notes` 純文字，沒有程式碼真的去判斷什麼時候該觸發槍械故障。
  4. 完全靠系統提示詞要求守密人「真的開槍要呼叫 adjust_ammo」，跟這個專案其他工具呼叫規則一樣沒有程式碼強制檢查——如果守密人忘記呼叫，彈藥數字就不會變、也不會有任何錯誤提示。
- **之後要擴充的話**：`/coc pc`／`/coc create` 建角時如果選了帶槍的職業，可以考慮跳出「要不要登記一把槍的彈藥」的提示；做槍械故障判定（COC7e 規則：擲出故障值以上視為卡彈/故障）；讓彈藥欄位也能透過指令手動調整，不用完全依賴 Keeper 自己呼叫工具。
